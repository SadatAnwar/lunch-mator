...
class RestClientWrapper @Inject()(ws: WSClient) {
  ...
  def post[T, R](url: String, headers: List[(String, String)], postData: T)(implicit fjs: Reads[R], wrt: BodyWritable[T], ec: ExecutionContext): Future[Option[R]] = {
  ...
  def get[A](url: String, headers: List[(String, String)] = List(), queryParams: List[(String, String)] = List())(implicit fjs: Reads[A], ec: ExecutionContext): Future[A] = {
  ...
  private def mapResponse[A](response: WSResponse)(implicit fjs: Reads[A], ec: ExecutionContext): Option[A] = {
